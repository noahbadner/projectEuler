...
def largest_product_in_series(n, series):
    ...
    :param int n: number of continuous factors
    :param String series: Series of integers to be searched
    :return int: the largest product of n continuous integers in the given series
    """
    largest_product = 0
    for i in range(len(series) - n + 1):
        substring = series[i:i + n]
        current_product = 1
        for digit in substring:
            current_product *= int(digit)
        largest_product = max(current_product, largest_product)
...
def main():
    ...
    print(largest_product_in_series(n, number_series))
